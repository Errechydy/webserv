...
void client_request(int client_socket)
{
    ...
    while((client_bytes_read = recv(client_socket, clientRequest + clientmsgsize, sizeof(clientRequest) - clientmsgsize - 1, 0)) > 0)
    {
       ...
       // printf("---> %zu\n", client_bytes_read);
        ...
        {
            clients[client_socket].end_of_req = 2;
            printf("22222 \n");
            break;
        }
        if(clientmsgsize > 300000 - 1 || (clientRequest[clientmsgsize-1] == '\n' && clientRequest[clientmsgsize-2] == '\r'))
        {
            clients[client_socket].end_of_req++;
            printf("111111 \n");
            break;
        }
    }
    clients[client_socket].req.append(clientRequest);
    // TODO: If  clients[client_socket].req contains "Content-Length", keep reading from socket Content-Length bytes recv 
    ...
    // printf("req1 : %s \n", clientRequest);
    // std::cout << "req2 :" << clients[client_socket].req << std::endl;



    if(clients[client_socket].end_of_req == 2)
    {
        
        clients[client_socket].reading = 1;
        clients[client_socket].end_of_req = 0;
    }
    exit(1);
...
void client_response(int client_socket)
{
        ...
        for (int i = 0; i < poll_size; i++)
        {
            ...
            if (poll_fds[i].revents & POLLIN) // Ready to read
            {
                ...
                else
                {
                    ...
                    // got_req = 1;
                    // exit(1);
                    client_request(poll_fds[i].fd);
                }
            }
            if ((poll_fds[i].revents & POLLOUT) && clients[poll_fds[i].fd].reading) // Ready to write
            {
                printf("POLLOUT =====> fd : %d, revents : %d\n", poll_fds[i].fd, poll_fds[i].revents);
                client_response(poll_fds[i].fd); // send the respons
                // got_req = 0;
                clients[poll_fds[i].fd].reading = 0;
