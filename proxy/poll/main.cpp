...
void handle_client_request(int client_socket)
{
    ...
    // int msgsentsize = 0;
    // int bytes_sent = 0;
    // while((bytes_sent = send(client_socket, hello + msgsentsize, sizeof(hello) - msgsentsize - 1, 0)))
    // {
    //     printf("---+-+-+-+-+-+-+-+-+-+----> %d\n", bytes_sent);
    //     msgsentsize += bytes_sent;
    //     if(msgsentsize == strlen(hello))
    //     {
    //         break;
    //     }
    // }
...
int main(int argc, char const *argv[])
{
    ...
    while(1)
    {
        ...
        for (int i = 0; i < poll_size; i++)
        {
            ...
            if (poll_fds[i].revents & POLLOUT) // Ready to write
            {
